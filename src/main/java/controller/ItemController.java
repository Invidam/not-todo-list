...
public class ItemController {
    ...
    public ResponseEntity<SuccessInfo> updateItem(@RequestBody ItemInfoDTO itemInfoDTO,@RequestHeader(value = "Authorization") String authHeader) {
        todoService.updateItemFromInfo(itemInfoDTO, authHeader);
        return ResponseEntity.ok(new SuccessInfo("UPDATE_SUCCESS", "success at update item."));
    ...
    public ResponseEntity<SuccessInfo> getItemById(@PathVariable long id, @RequestParam(value = "isDone") boolean isDone,  @RequestHeader(value = "Authorization") String authHeader) {
        todoService.updateItemStatus(new ItemIdAndIsDoneDTO(id,isDone),authHeader);
        return ResponseEntity.ok(new SuccessInfo("UPDATE_STATUS_SUCCESS", "success at update item status."));
